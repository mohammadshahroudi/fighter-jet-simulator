...
public static class MeshGenerator
{
	public static MeshData GenerateTerrainMesh(float[,] heightMap, MeshSettings meshSettings, int levelOfDetail)
	{
		...
		{
			Debug.LogError($"GenerateTerrainMesh: heightMap must be square, but was {heightMapWidth}x{heightMapHeight}.");
		}

		int numVertsPerLine = Mathf.Min(heightMapWidth, heightMapHeight);

		if (numVertsPerLine != meshSettings.numVertsPerLine)
		{
			Debug.LogWarning(
				$"GenerateTerrainMesh: meshSettings.numVertsPerLine ({meshSettings.numVertsPerLine}) " +
				$"did not match heightMap size ({heightMapWidth}x{heightMapHeight}). " +
				$"Using heightMap size {numVertsPerLine}."
			);
		}
		...
		for (int y = 0; y < numVertsPerLine; y++)
		{
			for (int x = 0; x < numVertsPerLine; x++)
			{
				...
				if (isEdgeConnectionVertex)
				{
					...
					Coord coordA = new Coord(
						(isVertical) ? x : x - dstToMainVertexA,
						(isVertical) ? y - dstToMainVertexA : y
					);

					Coord coordB = new Coord(
						(isVertical) ? x : x + dstToMainVertexB,
						(isVertical) ? y + dstToMainVertexB : y
					);

					bool coordAIsValid =
						coordA.x >= 0 && coordA.x < numVertsPerLine &&
						coordA.y >= 0 && coordA.y < numVertsPerLine;

					bool coordBIsValid =
						coordB.x >= 0 && coordB.x < numVertsPerLine &&
						coordB.y >= 0 && coordB.y < numVertsPerLine;

					if (!coordAIsValid || !coordBIsValid)
					{
						continue;
					}
					...
					EdgeConnectionVertexData edgeConnectionVertexData = new EdgeConnectionVertexData(
						vertexIndex,
						vertexIndicesMap[coordA.x, coordA.y],
						vertexIndicesMap[coordB.x, coordB.y],
						dstPercentFromAToB
					);
				...
				bool createTriangle =
					x + currentIncrement < numVertsPerLine &&
					y + currentIncrement < numVertsPerLine &&
